...
const LANGS = {
  fr: {
    ...
    help_whisper_body: 'Quand quelqu\'un résonne sur un de vos fantômes, votre téléphone vibre discrètement à l\'instant exact — pas de notification, juste une vibration mystérieuse.',
    help_reso_title: '✦ Résonance',
    help_reso_body: 'Une résonance par jour — choisissez le message qui vous touche. L\'auteur le ressentira via une vibration discrète.',
    help_empreinte_title: '🗺 Mon empreinte',
    help_empreinte_body: 'Dans votre profil, une carte personnelle trace tous les endroits où vous avez déposé ou découvert des fantômes.',
  ...
  en: {
    ...
    help_whisper_title: '✦ Ghost Whisper',
    help_whisper_body: 'When someone resonates on one of your ghosts, your phone vibrates discreetly at that exact moment — no notification, just a mysterious vibration.',
    help_reso_title: '✦ Resonance',
    help_reso_body: 'One resonance per day — pick the message that moves you. The author will feel it through a discreet vibration.',
    help_empreinte_title: '🗺 My footprint',
    help_empreinte_body: 'In your profile, a personal map traces all the places where you have deposited or discovered ghosts.',
...
function buildLeafletMap(centerLat, centerLng, h) {
  ...
  L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_nolabels/{z}/{x}/{y}{r}.png', { maxZoom: 20, className: 'tile-base' }).addTo(map);
  L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_only_labels/{z}/{x}/{y}{r}.png', { maxZoom: 20, className: 'tile-labels' }).addTo(map);
...
window.loadEmpreinteMap = async () => {
  ...
  try {
    ...
    L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_nolabels/{z}/{x}/{y}{r}.png', { maxZoom: 20, className: 'tile-base' }).addTo(_empreinteMap);
    L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_only_labels/{z}/{x}/{y}{r}.png', { maxZoom: 20, className: 'tile-labels' }).addTo(_empreinteMap);
...
window.showPublicProfileModal = (uid, name, ghostCount, totalOpens, ghostDocs) => {
  ...
  setTimeout(() => {
    ...
    L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_nolabels/{z}/{x}/{y}{r}.png', { maxZoom: 19, className: 'tile-base' }).addTo(pubMap);
    L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_only_labels/{z}/{x}/{y}{r}.png', { maxZoom: 19, className: 'tile-labels' }).addTo(pubMap);
...
window.setChainMarker = () => {
  ...
  const initChainMap = () => {
    ...
    L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_nolabels/{z}/{x}/{y}{r}.png', { maxZoom: 20, className: 'tile-base' }).addTo(cmap);
    L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_only_labels/{z}/{x}/{y}{r}.png', { maxZoom: 20, className: 'tile-labels' }).addTo(cmap);
...
function _initDepositMiniMap() {
  ...
  L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_nolabels/{z}/{x}/{y}{r}.png', {
    maxZoom: 20, className: 'tile-base'
  }).addTo(_depositMiniMap);
  L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_only_labels/{z}/{x}/{y}{r}.png', {
    maxZoom: 20, className: 'tile-labels'
  }).addTo(_depositMiniMap);
